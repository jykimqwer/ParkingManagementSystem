...
namespace prj_admin
{
    public partial class admin_Menu : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e) // 월별 조회 (총사용량 조회탭)
        {
            ...
            {
                string selectedMonth = monthForm.selectMonth;
                string query = "SELECT car_num, entry_time, exit_time, usage_time, parking_fee FROM cplate WHERE MONTH(exit_time) = {selectedMonth};";
                MySql_data(query);
                totalLabel.Text = $"이용차량 수 : {queryCount} 대";
                costLabel.Text = $"총 정산금액 : {totalCost} 원";
            }
